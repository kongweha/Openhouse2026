...
- Station artwork is not preloaded on the login screen. The seven visible state images begin asynchronous decoding only after a successful login, avoiding input lag from decoding fourteen large WebP assets while the participant enters a code. Persistent `will-change` layer hints were removed from Stamp controls to reduce compositor memory pressure.
